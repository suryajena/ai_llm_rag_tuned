...
class SqlProcessor(AbstractFileProcessor):
    def process_file(self, file_path: str, application_name: str, config: Dict) -> List[Dict]:
        """Process a SQL file and return chunked metadata."""
        try:
            ...
            logger.info(f"Processed SQL file: {file_path}, {len(metadata)} chunks")
            return metadata
        except IOError as e:
            logger.error(f"Failed to process SQL file: {file_path}", exc_info=True)
